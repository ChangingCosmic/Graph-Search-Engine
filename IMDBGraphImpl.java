...
public class IMDBGraphImpl implements IMDBGraph {
	...
	public static final String IMDB_DIRECTORY = "C:/Users/books/OneDrive/Desktop/WPI/CS 2103 (JAVA)/Starter Projects, etc/Project2_expanded";
		...
		while (s.hasNextLine()) {
			...
			if (fields.length >= NUM_REQUIRED_FIELDS) {
				...
				if (profession.contains("actor") || profession.contains("actress")) {
					...
					// add to _actorNamesToNodes
					_actorNamesToNodes.put(finalName, actorNode);

					for (String title : knownFor) {
						if (_movieNamesToNodes.get(idsToTitles.get(title)) != null) {
							actorNode.getNeighbors().add(_movieNamesToNodes.get(idsToTitles.get(title)));
						}
					}

					for (IMDBNode movie : actorNode.getNeighbors()) {
						movie.getNeighbors().add(actorNode);
					}
	...
	public static void main(String[] args) {
		try {
			System.out.println(IMDB_DIRECTORY + "/name.basics.tsv.gz");
			final IMDBGraph graph = new IMDBGraphImpl(IMDB_DIRECTORY + "/name.basics.tsv/data.tsv",
					IMDB_DIRECTORY + "/title.basics.tsv/data.tsv");
